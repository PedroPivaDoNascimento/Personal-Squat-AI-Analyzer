...
class VideoProcessor():
    ...
    def crop_roi(self, frame, norm_x, norm_y, sz_crop=50):
        ...
        y_min, y_max = max(0, px_y - radius), min(img_h, px_y + radius)
        x_min, x_max = max(0, px_x - radius), min(img_w, px_x + radius)
    ...
    def count_white_pixels(self, frame_atual):
        """Conta pixels de diferença em relação ao primeiro frame"""
        if frame_atual is None: 
            return 0
        
        if self.first_frame is None:
            self.first_frame = frame_atual.copy()
            return 0
        
        if self.first_frame.shape != frame_atual.shape:
            frame_atual = cv.resize(frame_atual, (self.first_frame.shape[1], self.first_frame.shape[0]))

        diff = cv.absdiff(self.first_frame, frame_atual)
        gray_image = cv.cvtColor(diff, cv.COLOR_BGR2GRAY)
        _, binary_image = cv.threshold(gray_image, 50, 255, cv.THRESH_BINARY)
