...
class LeftSaggital(BaseSaggital):
    def _calibrate_and_validate_with_height(self, dict_lm):
        ...
        try:
            ...
            normalized_person_height = abs(dict_lm['nose_y'] - dict_lm['left_heel_y'])
            
            # Comprimento da tíbia no vídeo (normalizado)
            normalized_tibia_length = VectorCalculator.calculate_distance(
                dict_lm['left_knee_x'], dict_lm['left_knee_y'],
                dict_lm['left_ankle_x'], dict_lm['left_ankle_y']
            )
    ...
    def _detect_repetition_phase(self, dict_lm, ts):
        ...
        showlder_x = dict_lm['left_shoulder_x']
        hip_x = dict_lm['left_hip_x']
        knee_x = dict_lm['left_knee_x']
        knee_y = dict_lm['left_knee_y']
    ...
    def create_dictionary_landmarks(self, lm_obj):
        try:
            return {
                ...
                'left_hip_x': lm_obj[23].x,
                'left_knee_x': lm_obj[25].x,
                'left_knee_y': lm_obj[25].y,
                'left_ankle_x': lm_obj[27].x,
                'left_ankle_y': lm_obj[27].y,
                'left_eye_x': lm_obj[2].x,
                'right_eye_x': lm_obj[5].x,
                'left_ear_x': lm_obj[7].x,  
                'left_big_toe_x': lm_obj[31].x,
                'left_heel_x': lm_obj[29].x,
                'left_shoulder_y': lm_obj[11].y,
                'right_shoulder_y': lm_obj[12].y,
                'left_hip_y': lm_obj[23].y,
                'left_knee_y': lm_obj[25].y,
                'left_ankle_y': lm_obj[27].y,
                'left_eye_y': lm_obj[2].y,
                'right_eye_y': lm_obj[5].y,
                'left_ear_y': lm_obj[7].y,
                'left_big_toe_y': lm_obj[31].y,
                'left_heel_y': lm_obj[29].y,
                'nose_x': lm_obj[0].x,
                'nose_y': lm_obj[0].y,
            }
        except Exception as e:
            print(f"Erro ao criar dicionário de landmarks espelhado: {e}")
            return {}
    ...
    def _check_head_posture_error(self, dict_lm, timestamp_ms):
        ...
        try:
            ear_x = dict_lm['left_ear_x']
            ear_y = dict_lm['left_ear_y']
            eye_x = dict_lm['left_eye_x']
            eye_y = dict_lm['left_eye_y']
            ...
            if (timestamp_ms/1000 >= 4 and timestamp_ms/1000 < 5):
    ...
    def _check_trunk_flexion_error(self, dict_lm, timestamp_ms):
        ...
        try:
            left_hip_x = dict_lm['left_hip_x']
            left_hip_y = dict_lm['left_hip_y']
            left_shoulder_x = dict_lm['left_shoulder_x']
            left_shoulder_y = dict_lm['left_shoulder_y']
            left_ankle_x = dict_lm['left_ankle_x']
            left_ankle_y = dict_lm['left_ankle_y']
            left_knee_x = dict_lm['left_knee_x']
            left_knee_y = dict_lm['left_knee_y']
            ...
            line_equation_trunk = VectorCalculator.get_line_equation(left_hip_x, left_hip_y, left_shoulder_x, left_shoulder_y)
            line_equation_tibia = VectorCalculator.get_line_equation(left_ankle_x, left_ankle_y, left_knee_x, left_knee_y)
            ...
            limit_of_x = -2
            if (((intersection_point[0] < min(left_hip_x, left_shoulder_x)) or (intersection_point[0] < min(left_ankle_x, left_knee_x))) and (intersection_point[0] > limit_of_x)):
    ...
    def _check_knee_translation_error(self, dict_lm, timestamp_ms):
        kn_status = 0
        try:
            foot_length_x = abs(dict_lm['left_big_toe_x'] - dict_lm['left_heel_x'])

            allowed_forward_translation = foot_length_x * 0.19  # Permite até 19% do comprimento do pé como avanço do joelho
            avancou_percentual = (dict_lm['left_knee_x'] - dict_lm['left_big_toe_x']) / foot_length_x
            avancou_exessivamente = (avancou_percentual < -1.0)

            #if ((timestamp_ms/1000 >= 5 and timestamp_ms/1000 < 6) or (timestamp_ms/1000 >= 9 and timestamp_ms/1000 < 10)):
    ...
    def _check_big_toe_lower_heel(self, dict_lm):
        ...
        try:
            if dict_lm['left_big_toe_y'] > (self.heel_y_inicial + TOLERANCIA_Y):
                print("Dedo do pe abaixou")
            return dict_lm['left_big_toe_y'] > (self.heel_y_inicial + TOLERANCIA_Y)
        except KeyError as e:
    ...
    def _check_heel_and_ankle_proximity(self, dict_lm):
        ...
        try:
            ...
            # Calcula a distância atual entre os dois pontos
            ...
            if current_distance < (self.initial_heel_ankle_distance * 1):
                print("O calcanhar está muito perto do tornozelo")
    ...
    def _check_heel_upper_ankle(self, dict_lm):
        try:
            if (dict_lm['left_heel_y'] < self.ankle_y_inicial):
                print("Calcanhar alto até de mais")
            return dict_lm['left_heel_y'] < self.ankle_y_inicial
        except KeyError as e:
            print(f"Erro: Landmarks necessários para a verificação de proximidade não foram encontrados: {e}")
            return False
    ...
    def _check_heel_lift_error(self, dict_lm, timestamp_ms=0):
        ...
        try:
            posicao_x_calcanhar = dict_lm["left_heel_x"]
            posicao_y_calcanhar = dict_lm["left_heel_y"]
            posicao_x_tornozelo = dict_lm["left_ankle_x"]
